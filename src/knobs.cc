...
namespace knob
{
	...
	/* coop_pythia */
	bool coop_rl_enable = false;
	float coop_rl_alpha_high_ipc = 0.05;
	float coop_rl_alpha_low_ipc  = 0.01;
	uint8_t coop_rl_ipc_threshold = 10;
	bool coop_enable_multicore = true;
	bool coop_enable_bw_throttling = true;
	bool coop_enable_adaptive_alpha = true;
	uint32_t coop_rl_bw_threshold = 4;
...
int parse_knobs(void* user, const char* section, const char* name, const char* value)
{
	...
	else if (MATCH("", "coop_rl_enable"))
	{
		knob::coop_rl_enable = !strcmp(value, "true") ? true : false;
	}
	else if (MATCH("", "coop_rl_alpha_high_ipc"))
	{
		knob::coop_rl_alpha_high_ipc = atof(value);
	}
	else if (MATCH("", "coop_rl_alpha_low_ipc"))
	{
		knob::coop_rl_alpha_low_ipc = atof(value);
	}
	else if (MATCH("", "coop_rl_ipc_threshold"))
	{
		knob::coop_rl_ipc_threshold = atoi(value);
	}
	else if (MATCH("", "coop_enable_multicore"))
	{
		knob::coop_enable_multicore = !strcmp(value, "true") ? true : false;
	}
	else if (MATCH("", "coop_enable_bw_throttling"))
	{
		knob::coop_enable_bw_throttling = !strcmp(value, "true") ? true : false;
	}
	else if (MATCH("", "coop_rl_bw_threshold"))
	{
		knob::coop_rl_bw_threshold = atoi(value);
	}
