...
bool CACHE::check_mshr_hit(uint64_t addr) {
    for (uint32_t i = 0; i < MSHR.occupancy; ++i) {
        if (MSHR.entry[i].address == addr)
            return true;
    }
    return false;
}

bool CACHE::check_pq_hit(uint64_t addr) {
    for (uint32_t i = 0; i < PQ.occupancy; ++i) {
        if (PQ.entry[i].address == addr)
            return true;
    }
    return false;
}
