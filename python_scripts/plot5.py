...
def plot_stacked_chart(df, title, output_file):
    ...
    for i, exp in enumerate(experiments):
        ...
        for j, x_val in enumerate(xpos):
            ax.text(x_val, -0.02, core_labels[j], ha='center', va='top', fontsize=16, rotation=90)
    ...
    for i, exp in enumerate(experiments):
        ...
        ax.text(xpos, -0.02, exp, ha='center', va='top', fontsize=16, rotation=90)

    max_val = df[["Covered", "Overpredicted", "Uncovered"]].sum(axis=1).max()
    ax.text(avg_label_pos, max_val + 0.05, "AVG", ha='center', va='bottom', fontsize=18, fontweight='bold')

    ax.set_ylabel(f'{title} Coverage Breakdown', fontsize=22)
    ax.set_title(f'{title} Prefetch Breakdown: Covered + Overpredicted + Uncovered', fontsize=24)
    ax.set_ylim(0, max_val + 0.2)
    ax.tick_params(axis='y', labelsize=18)
    ax.set_xticks([])
    ax.grid(axis='y', linestyle='--', linewidth=0.5)
    ax.legend(['Covered', 'Overpredicted', 'Uncovered'], fontsize=16)
...
# === MAIN EXECUTION ===
files = glob.glob("rollup-*-core.csv")  # Adjust if needed

llc_df = calculate_full_components(files, "LLC")
l2c_df = calculate_full_components(files, "L2C")

# Export CSVs
llc_df.to_csv("llc_prefetch_breakdown.csv", index=False)
l2c_df.to_csv("l2c_prefetch_breakdown.csv", index=False)

# Save PNGs
plot_stacked_chart(llc_df, "LLC", "llc_prefetch_breakdown.png")
plot_stacked_chart(l2c_df, "L2C", "l2c_prefetch_breakdown.png")
